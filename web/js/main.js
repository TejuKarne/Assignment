...
addtaskbtn.addEventListener("click", function(){
    ...
    if(addtaskinputval.trim()!=0){
        ...
        taskObj.push({'task_name':addtaskinputval, 'addtaskdateval':addtaskdateval, 'completeStatus':false});
...
function showtask(){
    ...
    taskObj.forEach((item, index) => {
        ...
        if(item.completeStatus==true){
            ...
            taskCompleteValue1 = `<td class="completed">${item.addtaskdateval}</td>`;
        }else{
            taskCompleteValue = `<td>${item.task_name}</td>`;
            taskCompleteValue1 = `<td>${item.addtaskdateval}</td>`;
        }
        html += `
                <tr>
                    <th  scope="row">${index+1}</th>
                    ${taskCompleteValue}
                    <td >${taskCompleteValue1}</td>
